...
mod tests {
    ...
    // ---------- PTH end-to-end over the reader ----------

    /// Raw `data.pkl` bytes extracted from anamnesis's real
    /// `tests/fixtures/pth_reference/algzoo_rnn_small.pth` fixture — a
    /// genuine `torch.save()`-produced pickle stream (448 bytes), not a
    /// hand-rolled one. Describes 3 `F32` tensors: `rnn.weight_ih_l0`
    /// `[2, 1]`, `rnn.weight_hh_l0` `[2, 2]`, `linear.weight` `[2, 2]` (see
    /// that fixture's sibling `algzoo_rnn_small_reference.json`). Reusing a
    /// real pickle stream avoids hand-crafting pickle VM opcodes — the same
    /// reasoning anamnesis's own `bench_pth_inspect` fixture uses. The
    /// surrounding ZIP archive below is synthetic, with a large dummy
    /// storage entry the front-matter parse must never fetch.
    const ALGZOO_RNN_SMALL_DATA_PKL: &[u8] = &[
        ,
    ];
    ...
    fn pth_front_matter_over_range_reader_reads_metadata_not_data() {
        ...
        let front = anamnesis::parse_pth_front_matter_from_reader(&mut reader)
            .expect("real algzoo data.pkl must parse cleanly");

        assert_eq!(front.tensors.len(), 3);
        let names: Vec<&str> = front.tensors.iter().map(|t| t.name.as_str()).collect();
        assert!(names.contains(&"rnn.weight_ih_l0"), "names: {names:?}");
        assert!(names.contains(&"rnn.weight_hh_l0"), "names: {names:?}");
        assert!(names.contains(&"linear.weight"), "names: {names:?}");
        let hh = front
            .tensors
            .iter()
            .find(|t| t.name == "rnn.weight_hh_l0")
            .unwrap();
        assert_eq!(hh.shape, vec![2, 2]);
        assert!(!front.big_endian);

        // The efficiency property this module exists for: metadata-only
        // transfer, a small number of requests, no bulk storage data
        // fetched.
        let stats = reader.stats();
        assert!(
            stats.requests <= 8,
