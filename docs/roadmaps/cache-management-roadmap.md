...
### v0.11.3 — Edition 2024 & dependency hygiene ✓

| Feature | Scope |
|---------|-------|
| Rust edition 2021 → 2024 ✓ | hf-fm was the last of the three co-developed crates still on 2021 (anamnesis and hypomnesis were already on 2024). The full `rust-2024-compatibility` lint group surfaced exactly one category — 19 `tail_expr_drop_order` sites, every one a `while let Some(x) = <expr>.await` loop or async-fn tail, none holding a lock or guard across the changed drop point. The one real work item was **28** `clippy::collapsible_if` sites collapsed into let-chains (a prior audit's "16" was the lib-only count; the true split is 16 lib / 11 binary / 1 test). `cargo fmt` also applied the 2024 style edition, which usefully corrected the indentation of several trailing `// BORROW:` / `// INDEX:` annotation comments. |
| `hypomnesis` feature-union fix ✓ | hf-fm declared `hypomnesis` bare, and **hypomnesis 0.2.8 made `cli` a default feature**; Cargo unions features graph-wide, so hf-fm was force-enabling `clap` + `ctrlc` for every consumer and **overriding candle-mi's own opt-out**. Latent rather than new — `^0.2.5` already admitted 0.2.9 and only the lockfile held it back. Fixed with `default-features = false` plus an explicit GPU-source feature list on both the runtime and dev-dependency entries. Library build 196 → 180 crates. |
| `hf-hub` 0.5 → 1.0, `reqwest` 0.12 → 0.13 ✓ | `hf-hub` 1.0 reached stable on crates.io, which was the standing precondition for the deferred `reqwest` bump (1.0 requires `reqwest ^0.13`, so they move together). A port, not a bump: 19 compile errors — `hf_hub::api::tokio` gone, `(owner, name)` addressing, `bon` builders, one collapsed `HFError`, `siblings` now `Option<Vec<_>>`, and the revision moved off the repo handle onto each call (absorbed by the new `repo::ModelRepo`). Breaking for `FetchError::Api`'s inner type and the four functions taking a repo handle; candle-mi uses none of them. |
| MSRV 1.88 → 1.91 ✓ | A consequence of `hf-hub` 1.0's mandatory `hf-xet` dependency — and a lesson: **`cargo`'s MSRV check under-reports the real floor**. Cargo saw only declared `rust-version` fields (1.89, via `konst ^0.4` / `redb ^3.1`), but `xet-core-structures` declares none and calls `str::floor_char_boundary`, stable since 1.91, so 1.89 and 1.90 fail with an unpredicted `E0658`. The floor was established by **bisection** (1.90 fails, 1.91 passes); the CI lane's comment records the method. |
| `anamnesis` 0.7.1 → 0.7.3 + dep sweep ✓ | Phases 7.2 / 7.3 upstream, neither touching an API hf-fm calls. Plus 119 semver-compatible transitive updates. |

**Theme: paying down the substrate before the last format lands.** No user-facing features. This release exists because three separate pieces of housekeeping all came due at once — the edition, a real supply-chain defect leaking a CLI-only feature into downstream consumers, and the `hf-hub` 1.0 stable release finally unblocking the `reqwest` 0.13 bump that had been deferred since 2026-05-19. Doing them together, before the remote-PTH work, keeps the v0.11.4 diff about PTH rather than about dependencies. Verified beyond the suite with a cold-cache live download, a forced 4-connection chunked download, and a remote Range inspect confirming the same `4 range requests, 8.0 KiB fetched` figure v0.11.1 recorded.

### v0.11.4 — Remote PTH inspect ✓

| Feature | Scope |
|---------|-------|
| `parse_pth_front_matter_from_reader<R: Read + Seek>` (anamnesis library work) ✓ | Not the originally-scoped `inspect_pth_from_reader` refactor — that had already shipped, but (as with GGUF's v0.11.2 detour) it turned out to be summary-only. A new `PthFrontMatter` type (`tensors: Vec<PthTensorInfo>`, `big_endian`) + `parse_pth_front_matter_from_reader(_with_limits)` pair exposes the full per-tensor list over any `Read + Seek` source, reusing the exact same `data.pkl`-materialise-via-central-directory + pickle-interpreter core `inspect_pth_from_reader` already used — no new parsing logic, no pickle-VM or security-boundary changes. Shipped as anamnesis `0.7.5`. |
| `inspect <repo> file.pth` (no `--cached`) ✓ | PTH dispatch wired through `HttpRangeReader` (`inspect::inspect_pth`, mirroring `inspect_npz` / `inspect_gguf`'s cache-first shape exactly, via a new shared `pth_tensor_info_to_header_info` mapping function reused by both the cached and remote paths). |
| `--tree` tolerates one edge outlier per numeric-sibling range ✓ | Not originally scoped — found live-verifying `--tree` against `RWKV/RWKV7-Goose-World-PTH`: `blocks.0` fuses the input-embedding `LayerNorm` and lacks `blocks.1`–`11`'s `att.v0`/`v1`/`v2` value-residual tensors, so the pre-existing all-or-nothing collapse rule refused to collapse `blocks.[0..11]` at all. `try_collapse_range` now tries a leading and/or trailing single-sibling outlier before giving up, rendering `blocks.0` standalone next to `blocks.[1..11]   (×11)` instead of 12 fully-expanded blocks. Scoped to edge outliers only (never the middle) — see `src/bin/main.rs`'s doc comment on `try_collapse_range` for why. Validated on a second, unrelated model family at far larger scale: [`docs/dogfooding-feedbacks/numeric-sibling-collapse-edge-outlier-validation.md`](../dogfooding-feedbacks/numeric-sibling-collapse-edge-outlier-validation.md) — `ModelCloud/DeepSeek-V2-Lite-gptq-4bit`'s `first_k_dense_replace=1` MoE architecture (layer 0 dense, layers 1–26 each with 64 experts) collapses from 20,834 tensors to 109 tree lines, with the nested `experts.[0..63]   (×64)` range collapsing recursively inside it. The same report also live-refutes a plausible-but-wrong hypothesis (mixed-precision GPTQ quantization at the edges) against `Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4` — its `I32`/`F16` split is uniform across all 28 layers, so it collapses fully with no outlier. Further validation swept three more MoE families (`Qwen3-30B-A3B`, `OLMoE`, older `Qwen2-57B-A14B`, all uniform — confirming `first_k_dense_replace` is DeepSeek-specific, not a general MoE trait) and one deliberate negative control (`ai21labs/Jamba-tiny-dev`'s hybrid Mamba/Attention stack, periodic not edge-bounded — correctly falls back to full expansion). |
| `--tree` numeric-sibling ordering bug (found via the Jamba negative control) ✓ | Uncollapsed numeric siblings (≥10 of them) rendered in `BTreeMap`'s lexicographic string order (`0, 1, 10, 11, ..., 2, 3, ...`) instead of numeric order — invisible below 10 siblings and invisible whenever collapse succeeds (`try_collapse_range` already re-sorts numerically before checking collapsibility), so only a full-collapse-failure case with ≥10 siblings exposed it. `trie_to_tree` now numeric-sorts a purely-numeric-segmented sibling group once, at tree-build time, fixing both the collapse-success and collapse-failure paths at the source. New regression test `tree_children_sort_numerically_even_when_collapse_fails` (verified to fail without the fix by temporarily reverting it); live re-verified against `ai21labs/Jamba-tiny-dev`. |
| `--check-gpu` repo-level full-table dump (found cross-version-testing v0.11.2/v0.11.3 via `git worktree`) ✓ | `--check-gpu` alone (no `--tree` / `--dtypes` / `--limit`) forces `run_inspect_repo`'s aggregation path to sum weight bytes precisely, but `run_inspect_repo_aggregated` had no compact branch for that case — it fell through to the full `print_multi_shard_table`, 20,848 lines on `DeepSeek-V2-Lite-gptq-4bit` before the four-line verdict. Confirmed present in v0.11.2 and v0.11.3 (built via `git worktree`, not just the working tree) — a pre-existing gap since `--check-gpu` shipped in v0.10.1, invisible until tested against a many-tensor MoE checkpoint. Now routes `!tree && !dtypes && limit.is_none()` (with or without `--filter`) through the same compact `print_multi_file_summary` rollup the non-aggregation bare path already used; `--limit` stays on the table path (no truncation support in the rollup). New regression test `inspect_cached_sharded_check_gpu_shows_compact_rollup_not_full_table` (verified to fail without the fix). |

**Closes the matrix.** Every tensor format anamnesis supports is now remotely inspectable via the same `HttpRangeReader` substrate. After v0.11.4, `hf-fm inspect <repo> <any-tensor-file>` works uniformly — cached or remote, regardless of format.

**Same design detour as v0.11.2, this time flagged before implementation instead of during.** Investigation found anamnesis 0.7.4's `inspect_pth_from_reader` (like `inspect_gguf_from_reader` before it) returns only the aggregate `PthInspectInfo` — no per-tensor list, which every hf-fm rendering flag (`--tree` / `--dtypes` / `--filter` / `--limit`) needs. Rather than implement the anamnesis-side fix inline, the gap was written up as a dogfooding report (`docs/dogfooding-feedbacks/pth-front-matter-for-hf-fm-remote-inspect.md`, filed against the anamnesis repo — cross-repo dogfooding, same pattern as [`hypomnesis-adoption.md`](../dogfooding-feedbacks/hypomnesis-adoption.md) but in the opposite direction, hf-fm reporting to anamnesis instead of the reverse) so the maintainer could implement anamnesis 0.7.5 in its own session against its own release cadence, deliberately not folded into the concurrently-planned anamnesis 0.8.0 (Python bindings). anamnesis 0.7.5 shipped exactly the asked-for shape, plus a bonus: its own consistency pass caught a real bug in the new `build_pth_tensor_info` reduction (an element-count fold that could report `byte_len: usize::MAX` for a zero-element tensor with an overflowing leading dimension, fixed before hf-fm ever depended on the release).

**Status: shipped.** `inspect::inspect_pth` follows `inspect_npz` / `inspect_gguf`'s exact shape: cache-first via `resolve_cached_path`, then `HttpRangeReader::open` + `spawn_blocking` running `anamnesis::parse_pth_front_matter_from_reader` on the reader, with the typed transport error preferred over the parse error on failure (keeps the gated-repo 401/403 diagnosis alive). The old synchronous `is_pth && !cached` rejection in `run_inspect_single` is gone; the `tests/cli.rs` test that asserted it (`inspect_remote_pth_errors_names_its_release`) is replaced by `inspect_remote_pth_reaches_the_network_layer`, which asserts the opposite — a remote `.pth` inspect against a nonexistent repo now fails with a network-shaped error, not the old static message. New offline unit test `pth_front_matter_over_range_reader_reads_metadata_not_data` in `src/http_range.rs` reuses a **real** 448-byte `data.pkl` pickle stream extracted from anamnesis's own `tests/fixtures/pth_reference/algzoo_rnn_small.pth` fixture (rather than hand-rolling pickle VM opcodes — the same reasoning anamnesis's own `bench_pth_inspect` fixture uses) wrapped in a synthetic ZIP archive with a 600 KiB dummy storage entry the parse must never fetch. **Live-verified against a real repo** (manual smoke test, not a `cargo test` target, matching the NPZ/GGUF precedent): `RWKV/RWKV7-Goose-World-PTH`'s uncached `RWKV-x070-World-0.1B-v2.8-20241210-ctx4096.pth` (364.49 MiB, 399 `BF16` tensors) reads as `Source: remote (12 range requests, 113.7 KiB fetched)` — 113.7 KiB of 364.49 MiB, well under 1% of the file — with `--filter` and `--json` rendering identically to the cached path. `--tree` on this same file is what surfaced the edge-outlier collapse gap above: `blocks.0` (input-embedding `LayerNorm` fused in) is now rendered standalone next to `blocks.[1..11]   (×11)`, confirmed via 7 new offline unit tests (full-range regression, each edge case, both edges, middle-outlier refusal, and aggregate-total correctness) plus this live re-verification after the fix.

### v0.11.5 — `hf-fm peek` (arbitrary small-file fetch, no anamnesis dispatch) ✓
...
**Out of scope for v0.11.5 (deferred):**
...
**Actual effort exceeded the estimate**, mostly in tests and rustdoc rather than logic: the generic, offline-testable core (`stream_peek` and its helpers, generic over any `Read + Seek`) landed as its own public library module, `src/peek.rs` (~935 lines including 22 unit tests and full `CONVENTIONS.md`-mandated doc comments; the actual streaming/validation logic is closer to the original ~55–75 LOC estimate). `flate2` was confirmed transitive (via `anamnesis`'s vendored ZIP handling, not `reqwest`) and promoted to direct per the plan above.

**Found and fixed along the way — not scoped, discovered via live testing.** The first live `peek` call against a real non-LFS file (`julien-c/dummy-unknown/README.md`) failed with an opaque `HTTP error: builder error`. Root cause: `chunked::probe_range_support` (shared by every `HttpRangeReader` consumer, including `inspect`) passed HF's redirect `Location` header straight to `reqwest::Client::get`, which requires an absolute URL — fine for every tensor format `inspect` targets (Git-LFS files always redirect to an absolute CDN URL) but broken for small, git-tracked files, which HF redirects through a **relative** `/api/resolve-cache/...` path. Purely latent since the v0.11.0 substrate shipped; invisible until `peek` became the first feature to Range-fetch a non-LFS file. Fixed with a new `resolve_redirect_url` helper (`Url::join` against the request's own URL, per RFC 9110 §10.2.2) plus 3 regression tests. Full write-up in `CHANGELOG.md`'s `[Unreleased]` → `Fixed` section.

**Status: implemented and live-verified against the real Hub** (`julien-c/dummy-unknown` for the core `--head`/`--tail`/`--max` matrix and the gated-repo diagnosis path, `bluelightai/clt-qwen3-1.7b-base-20k`'s real `features/index.json.gz` for `--gunzip`, both auto-detected and explicit) — 22 offline unit tests, 3 redirect-fix regression tests, 13 live CLI tests in `tests/cli.rs`, full suite green (`cargo fmt` / `clippy -D warnings` / `cargo test --all-features` / `cargo doc -D warnings`). Pending before tagging v0.11.5: rename `CHANGELOG.md`'s `[Unreleased]` → `[0.11.5]`, bump `Cargo.toml`/`Cargo.lock`.

---

### v0.12.0 — Backlog cleaning

**Theme.** v0.11.5 closes the v0.11.x line. A dedicated sweep of this roadmap, `CHANGELOG.md`, and the dogfooding reports at that close-out point turned up five items that were scoped, half-shipped, or explicitly deferred across the v0.10.x/v0.11.x lines and never came back for closure — plus one stale doc reference from the same sweep. None needs new hardware, so this phase takes the slot the multi-GPU matrix work would otherwise have claimed next; that work is pushed to v0.13.0 (below) to wait for a second GPU, so backlog cleaning fills the gap instead of leaving it idle.

| Item | Scope | Origin |
|---|---|---|
| `diff --collapse` flag | Numeric-segment / layer-index collapsing for `diff`'s per-tensor output — the tool-side counterpart to the `jq` recipe already shipped in `docs/FAQ.md` (v0.10.2). Needs a heuristic decision first (numeric-segment abstraction? template grouping? expert-routing-aware, given `--tree`'s v0.11.4 experience with MoE routing?); the "two or three real cases" the original note wanted to inform that choice were never tracked, so start there before designing the flag. | v0.10.2: *"the bake-in is deferred to v0.11 once two or three real cases land."* Never revisited. |
| `diff-config <REPO_A> <REPO_B>` | Architectural comparison between scaled-sibling pairs (e.g. a 35B vs. an 80B variant) — a `config.json` field-by-field diff, complementing `diff --dtypes`'s tensor-level view. | v0.10.4 deferred list, labeled `"v0.11 scope"`. Never revisited. |
| hypomnesis spilling-detection verdict adoption | hypomnesis 0.2.5 (bumped in v0.11.0) added VRAM over-commit / spilling detection as a `device_info` signal; `--check-gpu`'s fit verdict has never read it. Wire it into the verdict output (a `Spilling:` line, or fold into the existing fit/no-fit determination) — check the current hypomnesis API surface first, since two releases have passed since the bump. | v0.11.0: *"spilling detection; verdict-side adoption deferred."* Still unused as of v0.11.5. |
| Flaky `tests/cli.rs` pair | `dry_run_cached_repo_shows_zero_download` and `cache_delete_nonexistent_repo` share cache state and pass only by scheduling luck under parallel test execution. Fix: self-sufficient cache setup for both, `serial_test` as a new dev-dependency to serialise the ones that share a cache directory. Unblocks restoring `cargo test` to the MSRV CI lane, currently lint-only because of this exact flake. | `CHANGELOG.md` `[0.11.3]`, "Known issue" — fix already named, never applied. |
| `--tree` edge tolerance: `K > 1` outliers per edge | `try_collapse_range` tolerates exactly one structurally-different sibling per edge (v0.11.4). DeepSeek-V3's `first_k_dense_replace=3` needs three. Raise the tolerance to a small constant `K` (start at 2–3, not unbounded) and re-verify live against a DeepSeek-V3-class checkpoint if one becomes practical to round-trip (670B+ params / 160+ shards was the blocker at write time). | `numeric-sibling-collapse-edge-outlier-validation.md`: *"a natural, bounded extension... deliberately left as a documented, not-yet-needed follow-up."* Named, scoped, unverified. |
| `docs/FAQ.md` stale style-comment | The file's own maintenance-convention comment (top of file, "Scope" rule) still cites the `peek` subcommand as the example of "unshipped work" to avoid pre-documenting — but `peek` shipped in v0.11.5. One-line fix, doc-only, no behavior change. | Doc hygiene, surfaced by the same v0.11.5-closeout sweep. |

**Deferred out of this phase** (same sweep, lower priority, not broken promises): the GGUF-reader dequant parallelisation queued in anamnesis's own `ROADMAP.md` — cross-repo, not hf-fm's backlog to close; and `peek`'s remaining out-of-scope siblings (`Binary file rendering`, `Search / grep semantics`, `Range across multiple files`, `Cached file path`) — design boundaries drawn on purpose, not gaps.

No implementation order imposed — the six items are independent (different subsystems: `diff`, `gpu_check.rs`, test infra, `--tree`, docs) and can land in any sequence or in parallel across small commits.

---

### v0.12.1 — Quant-aware fit planning (VRAM-budget dogfooding)

**Theme.** Surfaced by the [Laguna VRAM-fit dogfooding session](../dogfooding-feedbacks/hf-fm-dogfooding-vram-fit-laguna-session.md) (2026-08-24, against v0.11.5): a pure consumer question — "which quant of `poolside/Laguna-XS-2.1` fits a 16 GiB card?" — took five separate `list-files` calls and hand assembly to answer, because every input `hf-fm` needed was already inside the tool and no command emitted the joined view. Unlike v0.12.0's backlog items, none of these five gaps were previously scoped anywhere in this roadmap — they're new-in-v0.12.1, not deferred-and-reclaimed. Four of the five compose into one feature seen from different angles (aggregate quant siblings, size them, compute what each would take to run); the fifth (header caching) is independent perf work. Like v0.12.0, nothing here needs a second GPU, so it can land immediately after v0.12.0 without waiting on the v0.13.0 hardware prerequisite.

| Item | Scope | Origin |
|---|---|---|
| `inspect --group-by <PATTERN>` tensor-name-class rollup | New rollup mode alongside the existing `--dtypes` histogram — `--dtypes` buckets by dtype, this buckets by a tensor-name glob (e.g. `blk.*.ffn_*_exps`), printing `MATCHED` vs `OTHER` byte totals and percentages, plus a per-MoE-layer average once composed with the model's layer count (e.g. `per-MoE-layer expert cost: 0.453 GiB`). Foundational: the `--fits` row below consumes this rollup directly rather than reimplementing it. | Gap 3, promoted mid-report from MEDIUM to HIGH once its role in `--fits` became clear: *"`inspect` has no rollup by tensor-name class"*. |
| `hf-fm quants <REPO_ID>` + `--fits <SIZE> [--reserve <SIZE>]` | Aggregates a base model's quant sibling repos — discovered via a GGUF `general.source.url` / `base_model` tag backlink, or the `<base>-<SCHEME>` naming convention — into one sorted table (artifact / size / repo / bits). `--fits` composes with it to emit a **plan, not a boolean**: for MoE checkpoints it uses the `--group-by` rollup to compute the minimum `--n-cpu-moe N` that keeps the resident footprint under budget (`N = ceil((W − budget + reserve) / (E/L))`), so a build 40% over the raw VRAM budget can still surface as viable via CPU offload instead of being rejected by a naive size comparison. | Gap 1 (HIGH), the report's defining gap — "no cross-repo quant view, and no offload-aware fit planning". Depends on the `--group-by` row above. |
| `list-files` / `du` quant-repo total-footer fix | Detects self-contained-multi-file quant repos (N mutually exclusive `.gguf` alternatives) versus sharded repos (one logical file split via `index.json`), and stops summing across the former — a min/median/max readout or a suppressed total, instead of `"31 files, 509.16 GiB total"` implying a single 509 GiB download nobody would make. | Gap 4 (LOW) — *"the `list-files` total footer is misleading for quant repos"*. |
| `search --show size` range output for multi-quant repos | A correction, not a new flag: `--show size` already shipped in v0.10.4 (closes [Gap 1 of the anamnesis Phase 5 report](../dogfooding-feedbacks/hf-fm-dogfooding-anamnesis-phase5.md)), so this report's framing — "the one field missing" — is inaccurate. What is genuinely still open is that `--show size` sums every file in the repo ([`fetch_repo_total_size`](../../src/discover.rs)), inheriting the exact category error the row above fixes for `list-files`: a 27-file quant repo renders one meaningless total. Reuses that row's shard-vs-quant detection to switch to a size range (`8.76 to 36.83 GiB (27 files)`) for detected quant repos. | Gap 2 (HIGH in the report; the "missing field" framing does not hold up against the code, so scope is narrowed here to the summation defect only). |
| Remote header cache keyed on `(repo, revision, filename, etag)` | Persists parsed `inspect` headers under the existing cache root so repeat inspection of the same remote file — the natural pattern of iterative narrowing over a handful of quant candidates — is free on the second and third call. `--cached` already establishes the vocabulary for controlling this. Independent of the four rows above. The 59 sequential range requests a single `inspect` call issues also look parallelizable; noted as a follow-on, not part of this item's scope. | Gap 5 (MEDIUM) — *"remote headers are not cached between invocations"*. |

**Implementation order.** Row 1 (`--group-by`) before row 2 (`quants` + `--fits`) is a hard dependency — the offload-plan arithmetic reads the rollup directly — and mirrors the report's own sequencing note: *"Gap 3 is the one to build first, because it is the only one that is purely local to a single header parse, and both of the others become straightforward once a byte rollup exists."* Rows 3 and 4 share one detection heuristic (self-contained multi-file quant repo vs. `index.json` shard set) and are natural to land together, in either order, independent of rows 1–2. Row 5 is independent perf work and can land at any point in the sequence.

**Relationship to v0.13.0.** `--fits`'s offload plan and `--check-gpu`'s device verdict are complementary, not overlapping: `--check-gpu` answers "does this one already-chosen file fit on this one GPU, at this context length"; `--fits` answers "which of N candidate quant files fits at all, and what CPU-offload split do the tight ones need" *before* a file is chosen. Composing them — `--fits` narrowing the candidates, `--check-gpu` confirming the winner against live free VRAM rather than the nominal budget passed to `--fits` — is a natural follow-up but out of scope here, since `--fits` takes a user-supplied byte budget, not a live device query.

---

### v0.13.0 — Multi-GPU + multi-context `--check-gpu` matrix
...
**Hypomnesis bump 0.2.1 → 0.2.3 — taken early in v0.10.5 (2026-06-12).** The original plan (below, kept for the record) was to hold at `0.2.1` until the v0.13.x `--check-gpu --processes` work, on the reasoning that 0.2.2's Windows `PDH` per-process backend was unused weight for a `device_info`-only consumer. That reasoning held for 0.2.2 — but **0.2.3 adds first-class macOS / Apple Silicon support, including the device-wide VRAM budget via `MTLDevice.recommendedMaxWorkingSetSize`, which is precisely the `device_info` figure `--check-gpu` reads.** That turns `inspect --check-gpu` from "no GPU detected" into a real fit verdict on a Mac — a `device_info` win the deferral never contemplated, so the bump was pulled forward. Cost on the hf-fm side was one line (`gpu_check.rs` `friendly_error` now handles the `HypomnesisError::Pdh` variant explicitly, satisfying `wildcard_enum_match_arm`); the new `objc2` / `objc2-metal` deps are `cfg(target_os = "macos")`-gated, so Linux/Windows builds are byte-identical. The per-process `gpu_processes()` path (PDH / Metal compute-process listing) remains unused until `--check-gpu --processes` lands in v0.13.x — that feature still drives the *next* hypomnesis interaction, just not the version pin.

> *Original deferral note (superseded by the 0.2.3 adoption above):* Through the v0.10.x `--check-gpu` line hf-fm pins `hypomnesis = "0.2.1"`: the KV/Mamba budgeting work reads only `device_info` (device-level total/free/used), which 0.2.2 does not change, while 0.2.2's headline addition — the Windows `PDH` per-process-VRAM backend for `gpu_processes()` — would compile unused into the binary. The bump's natural home is the v0.13.x GPU-reach line. Until then, pinning 0.2.1 keeps hf-fm's build free of a backend it does not exercise.

**Theme: broaden `--check-gpu` from a single verdict to a matrix.** v0.10.1 introduced single-device, weights-only `--check-gpu`; v0.10.4 added KV-cache and hybrid-Mamba budgeting at a user-supplied context. v0.13.0 closes out the family: instead of one device × one context giving one verdict, the user can ask for all devices × a context-length sweep and read off a table. Deferred to v0.13.0 because both features want real multi-GPU hardware to validate (the iteration ordering, the auto-pick heuristic, and the per-GPU sweep all need an actual second device on the box).

**Why v0.13 and not v0.11.x, v0.12.0, or v0.12.1.** The v0.11 line is dedicated to remote-inspect framework work (`HttpRangeReader` + format coverage) — stitching multi-GPU into v0.11 would dilute that focus. v0.12.0 was then claimed by backlog cleaning (above) rather than left idle while waiting on hardware, since none of that phase's six items need a second GPU either — and v0.12.1's quant-aware fit planning (above) follows the same reasoning, surfaced by dogfooding after the v0.12.0 scope was already fixed. That keeps each boundary clean: *v0.11 = format reach, v0.12.0 = backlog cleaning, v0.12.1 = quant/fit-planning reach, v0.13 = GPU reach* — and means v0.13.0 can slip freely against hardware availability without blocking anything else.
...
**Performance implications for the v0.13.0 pipeline-parallel use case.** For PP-style serving where the second GPU receives intermediate activations across a layer boundary:
...
**Conclusion.** Hardware plan is viable for the v0.13.0 work. Only material risk before purchase is **physical clearance with the existing 5060 Ti's cooler** — measure first. PSU, power delivery, BIOS, and chipset routing are all fine.

---

### `inspect` pickle-safety verdict — candidate (post-v0.13, gated on anamnesis)
...
**Provisional placement:** a small v0.13.x patch or the v0.14 opener once the anamnesis scan API lands; the version is deliberately unassigned until then.
