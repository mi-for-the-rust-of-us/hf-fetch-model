...
- **Outcome:** astorise confirmed the catch within hours ([comment](https://github.com/huggingface/candle/pull/3821#issuecomment-5217229466), 2026-08-07): "Good catch @PCfVW, confirmed by inspecting real Unsloth checkpoints." Describes a fix matching `"qwen35"` (keeping `"qwen3_5"` as an alias), flipping the canonical name to `"qwen35"`, and giving `"qwen35moe"` a distinct "not supported yet" error instead of the generic "unsupported gguf architecture" message, plus two new tests, citing commit `117c4d0`. Verified live: that commit exists in `astorise/candle` and genuinely implements all of it (checked its actual `quantized_lm.rs` content, not just the description). One open point: as of this check, `117c4d0` sits on a separate branch, `claude/candle-pr-3821-review-k703ya`, not on PR #3821's own branch (`astorise/issue-3820-qwen35-quantized-dispatch`), whose only commit since our post is a `Merge branch 'main'`. The fix is written and correct but has not yet landed on the PR itself.
