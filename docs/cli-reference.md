...
# MoE expert-byte rollup: what fraction of this GGUF is expert weights?
hf-fm inspect poolside/Laguna-XS-2.1-GGUF Q4_K_M.gguf --group-by 'blk.*.ffn_*_exps.weight'

# Group-by rollup as JSON (for scripting — e.g. hf-fm quants --fits's offload math)
hf-fm inspect poolside/Laguna-XS-2.1-GGUF Q4_K_M.gguf --group-by 'blk.*.ffn_*_exps.weight' --json
...
| `--filter` | Show only tensors whose name contains this substring (case-insensitive) | — |
| `--group-by PATTERN` | Roll tensors up into a MATCHED (name matches the `globset` glob `PATTERN`) / OTHER byte split instead of per-dtype or per-tensor output — e.g. `--group-by 'blk.*.ffn_*_exps.weight'` isolates GGUF MoE expert weights. Adds a `per-MoE-layer expert cost` line when exactly one numeric tensor-name segment varies among matched tensors (the layer index); omitted when zero or more than one position varies (ambiguous). Composes with `--filter` and `--limit` the same way `--dtypes` does — both narrow the tensor list before the rollup is computed. Composes with `--json` to emit `{ pattern, matched: {...}, other: {...}, total_tensors, total_params, total_bytes, layer_count, per_layer_bytes }`. Conflicts with `--dtypes` and `--tree`. | — |
...
| `--tree` | Show a hierarchical tree view grouped by dotted namespace prefix; numeric sibling groups with identical sub-structure collapse to `[0..N]`, tolerating up to 3 structurally-different siblings at each edge of the range (raised from 1 in v0.12.0, motivated by DeepSeek-V3's `first_k_dense_replace=3`; the original v0.11.4 tolerance of 1 covered e.g. a first block that fuses an extra input-embedding `LayerNorm`), each outlier rendered standalone next to the collapsed majority. Composes with `--filter` and `--json`. Conflicts with `--dtypes`, `--group-by`, and `--limit`. | off |
| `--revision` | Git revision (branch, tag, SHA) | main |
